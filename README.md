...
*   **Gatekeeper:** First evaluates the user prompt using a fast and cost-effective model (`gemini-2.5-flash` or `gpt-4o-mini`). It returns a structured JSON with a boolean flag `is_complex` and a brief description of the `reasoning`.
*   **Executor:**
    *   If `is_complex: False` (e.g., adding a simple single chore) -> the final JSON extraction is generated by the same cheap model.
    *   If `is_complex: True` (e.g., scheduling conflicts, multi-step planning, or complex dates) -> the agent **dynamically swaps** the model to a more powerful, premium tier (`gemini-2.5-pro` or `gpt-4o`) to perform the advanced reasoning.
...
2.  Open `.env` in your editor and choose your active provider with `AI_PROVIDER`:
    *   **Option A: Google Gemini (Recommended)**
        ...
        *   By default, the assistant uses `gemini-2.5-flash` for the cheap gatekeeper and `gemini-2.5-pro` for the premium executor.
    *   **Option B: OpenAI**
...
2.  **Gatekeeper Check:** The gatekeeper (`gemini-2.5-flash` / `gpt-4o-mini`) processes the prompt. Since it contains a timeline ("tomorrow", "later"), legal references ("2018 regulations"), and multiple chores, it outputs `is_complex: True`.
3.  **Model Routing Switch:** The backend detects the complex flag and redirects the request to the high-tier Executor (`gemini-2.5-pro` / `gpt-4o`).
...
🤖 [J.A.R.V.I.S. DYNAMIC ROUTING DECISION]
  ...
  Model Selected:       gemini-2.5-pro (EXPENSIVE tier)
🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
